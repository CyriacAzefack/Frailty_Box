...
def compute_pattern_accuracy(occurrences, period, description, start_date=None, end_date=None, tolerance_ratio=2):
    '''
    ...
    nb_description_components = len(description)
    ...
    if is_occurence_expected(relative_start_date, description, period, tolerance_ratio) is not None:
        bord_effects_expected_occ += 1

    if is_occurence_expected(relative_end_date, description, period, tolerance_ratio) is not None:
        bord_effects_expected_occ += 1
    ...
    occurrences["expected"] = occurrences["relative_date"].apply(
        lambda x: is_occurence_expected(x, description, period, tolerance_ratio))
